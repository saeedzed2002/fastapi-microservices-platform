...
def test_phase19_return_contracts_define_physical_and_financial_boundaries() -> None:
    ...
    assert contracts["order.returns.v1"]["status"] == "active"
    assert contracts["order.returns.v1"]["owner"] == "order-service"
    assert contracts["order.return_received.v1"]["status"] == "active"
    ...
    assert (
        "return_request_id"
        not in json.loads(
            (ROOT / "contracts" / "events" / "order.refund_requested.v1.schema.json").read_text(
                encoding="utf-8"
            )
        )["required"]
    )
    assert (
        "return_request_id"
        in json.loads(
            (ROOT / "contracts" / "events" / "payment.refunded.v1.schema.json").read_text(
                encoding="utf-8"
            )
        )["properties"]
    )
