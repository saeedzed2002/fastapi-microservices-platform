...
`queued`. An `admin` reads queue metadata,
then claims exactly one conversation. After a claim, only the customer and the
assigned administrator are Chat participants.
...
2. Call `POST /api/v1/chat/support/conversations/{conversation_id}/claim`.
   ...
   administrator already claimed or closed the conversation; refresh the queue instead
...
4. Call `POST /api/v1/chat/support/conversations/{conversation_id}/release`
   ...
   the current administrator's Chat membership.
...
- `403`: the access token lacks the `customer` or `admin` role required by the
  endpoint.
- `404` after another administrator claims: expected authorization behavior; the administrator
...
- `409` while sending: the assigned administrator closed the conversation. Create a
...
realtime delivery but cannot make another administrator a participant. If an administrator
