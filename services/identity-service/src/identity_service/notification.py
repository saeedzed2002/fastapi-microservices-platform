...
class NotificationPasswordResetGateway:
    """Create a durable email-delivery intent in Notification Service."""
    ...
    async def enqueue(self, *, delivery_id: UUID, email: str) -> None:
        if self._settings.internal_otp_shared_secret is None:
            raise NotificationUnavailable
        try:
            response = await self._client.post(
                ...
                headers={"X-Platform-Internal-Token": self._settings.internal_otp_shared_secret},
                json={"delivery_id": str(delivery_id), "email": email},
            )
            response.raise_for_status()
        except httpx.HTTPError as exc:
            raise NotificationUnavailable from exc
